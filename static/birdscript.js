...
$( document ).ready( function () {
	...
	function makeOrderDivs(sp_orders) {
		for (i = 0; i < sp_orders.length; i++) {
			console.log(sp_orders[i]);
	...
	function makeFamilyDivs(birds_dict) {
		allOrders = $(".order");
		for (i = 0; i < allOrders.length; i++) {
			families = birds_dict[allOrders[i].id];
			for( var family in families ) {
				console.log(family);
				var familyDiv = $('<div></div>');
				familyDiv.attr('id', family);
				familyDiv.addClass('family');
				$("#" + allOrders[i].id).append(familyDiv);
			}
		}
	}

	function makeSpeciesDivs(birds_dict) {
		allFamilies = $(".family");
		for (i = 0; i < allFamilies.length; i++) {
			allSpecies = birds_dict[allFamilies[i].id];
			for( var species in allSpecies ) {
				console.log(species);
				var speciesDiv = $('<div></div>');
				speciesDiv.attr('id', species);
				speciesDiv.addClass('species');
				$("#" + allFamilies[i].id).append(speciesDiv);
				}
			}
		}

	makeOrderDivs(sp_orders);
	makeFamilyDivs(birds_nest);
	makeSpeciesDivs(birds_nest);
